...
export class Selector {
  ...
  public static getRequestText(
    ...
    if (editor.selection.isEmpty || range) {
      const activeLine = !range
        ? editor.selection.active.line
        : range.start.line;
      selectedText = Selector.getDelimitedText(
    ...
    for (const current of delimitedLines) {
      ...
      while (start <= end) {
        ...
        if (
          ...
          Selector.isResponseStatusLine(startLine)
        ) {
          break;
        }

        if (
          (options.ignoreCommentLine && Selector.isCommentLine(startLine)) ||
          (options.ignoreEmptyLine && Selector.isEmptyLine(startLine)) ||
          (options.ignoreFileVariableDefinitionLine &&
            Selector.isVariableDefinitionLine(startLine))
        ) {
          start++;
          continue;
        }
        ...
        if (
          (options.ignoreCommentLine && Selector.isCommentLine(endLine)) ||
