...
import {
  fromString as ParsePreviewOptionStr,
  PreviewOption
} from "./previewOption";
...
export class RestClientSettings implements IRestClientSettings {
  ...
  public enableOAuth2CodeLens: boolean;
  ...
  private initializeSettings() {
    ...
    const restClientSettings = workspace.getConfiguration(
      "rest-client",
      document ? document.uri : null
    );
    this.followRedirect = restClientSettings.get<boolean>(
      "followredirect",
      true
    );
    this.defaultHeaders = restClientSettings.get<RequestHeaders>(
      "defaultHeaders",
      {
        "User-Agent": "vscode-restclient"
      }
    );
    this.showResponseInDifferentTab = restClientSettings.get<boolean>(
      "showResponseInDifferentTab",
      false
    );
    this.requestNameAsResponseTabTitle = restClientSettings.get<boolean>(
      "requestNameAsResponseTabTitle",
      false
    );
    this.rememberCookiesForSubsequentRequests = restClientSettings.get<boolean>(
      "rememberCookiesForSubsequentRequests",
      true
    );
    this.timeoutInMilliseconds = restClientSettings.get<number>(
    ...
    this.excludeHostsForProxy = restClientSettings.get<string[]>(
    ...
    this.environmentVariables = restClientSettings.get<
      Map<string, Map<string, string>>
    >("environmentVariables", new Map<string, Map<string, string>>());
    this.mimeAndFileExtensionMapping = restClientSettings.get<
      Map<string, string>
    >("mimeAndFileExtensionMapping", new Map<string, string>());

    this.previewResponseInUntitledDocument = restClientSettings.get<boolean>(
      "previewResponseInUntitledDocument",
      false
    );
    this.previewColumn = this.parseColumn(
      restClientSettings.get<string>("previewColumn", "two")
    );
    this.previewResponsePanelTakeFocus = restClientSettings.get<boolean>(
      "previewResponsePanelTakeFocus",
      true
    );
    this.hostCertificates = restClientSettings.get<
      Map<string, HostCertificate>
    >("certificates", new Map<string, HostCertificate>());
    this.disableHighlightResonseBodyForLargeResponse = restClientSettings.get<
      boolean
    >("disableHighlightResonseBodyForLargeResponse", true);
    this.disableAddingHrefLinkForLargeResponse = restClientSettings.get<
      boolean
    >("disableAddingHrefLinkForLargeResponse", true);
    this.largeResponseBodySizeLimitInMB = restClientSettings.get<number>(
      "largeResponseBodySizeLimitInMB",
      5
    );
    this.previewOption = ParsePreviewOptionStr(
      restClientSettings.get<string>("previewOption", "full")
    );
    this.formParamEncodingStrategy = ParseFormParamEncodingStr(
      restClientSettings.get<string>("formParamEncodingStrategy", "automatic")
    );
    this.enableTelemetry = restClientSettings.get<boolean>(
      "enableTelemetry",
      true
    );
    this.addRequestBodyLineIndentationAroundBrackets = restClientSettings.get<
      boolean
    >("addRequestBodyLineIndentationAroundBrackets", true);
    this.decodeEscapedUnicodeCharacters = restClientSettings.get<boolean>(
      "decodeEscapedUnicodeCharacters",
      false
    );
    this.logLevel = ParseLogLevelStr(
      restClientSettings.get<string>("logLevel", "error")
    );
    this.enableSendRequestCodeLens = restClientSettings.get<boolean>(
      "enableSendRequestCodeLens",
      true
    );
    this.enableCustomVariableReferencesCodeLens = restClientSettings.get<
      boolean
    >("enableCustomVariableReferencesCodeLens", true);
    languages.setLanguageConfiguration("http", {
      brackets: this.addRequestBodyLineIndentationAroundBrackets
        ? this.brackets
        : []
    });
